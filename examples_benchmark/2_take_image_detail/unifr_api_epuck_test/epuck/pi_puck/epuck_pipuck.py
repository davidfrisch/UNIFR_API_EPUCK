...
class PiPuckEpuck(Epuck):
    ...
    def get_camera_read(self):
        ...
        loop_img_time = time.time()
        for _ in range(5):
            self.camera.grab()

        success, frame = self.camera.retrieve()
        loop_img_time = time.time()-loop_img_time

        if success:
            #resize the image if end user asked to change it
            resize_img_time=0
            if self.__camera_width != 640 and self.__camera_height != 480:
                resize_img_time = time.time()
                frame = cv2.resize(frame, (self.__camera_width, self.__camera_height))
                resize_img_time = time.time() - resize_img_time
            
            with open('grab_results_160120.json', "r") as file:
                data = json.load(file)

                data['loop_img_time'] += [loop_img_time]
                data['resize_img_time'] += [resize_img_time]

            with open('grab_results_160120.json', "w") as file:    
                json.dump(data,file)
