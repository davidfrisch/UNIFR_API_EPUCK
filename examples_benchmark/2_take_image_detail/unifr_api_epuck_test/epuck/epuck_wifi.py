...
class WifiEpuck(Epuck):
    ...
    def __receive_from_robot(self):
        ...
        # camera information
        time_to_receive_camera_info = 0
        time_to_receive_sensor_info = 0
        if header == bytearray([1]):
            time_to_receive_camera_info = time.time()
            self.__rgb565 = self.__receive_part_from_robot(self.IMAGE_PACKET_SIZE)
            time_to_receive_camera_info = time.time()-time_to_receive_camera_info
            self.__camera_updated = True


        # sensors information
        elif header == bytearray([2]):
            time_to_receive_sensor_info = time.time()
            self.sensors = self.__receive_part_from_robot(self.SENSORS_PACKET_SIZE)
            time_to_receive_sensor_info = time.time()- time_to_receive_sensor_info
        # no information
        else:
            return False


        with open('results_120_wifi.json', "r") as file:
            data = json.load(file)

            data['time_to_receive_camera_info'] += [time_to_receive_camera_info]
            data['time_to_receive_sensor_info'] += [time_to_receive_sensor_info]

        with open('results_120_wifi.json', "w") as file:    
            json.dump(data,file)
...
# definition of roll and pitch https://www.youtube.com/watch?v=5IkPWZjUQlw
    ...
    def get_camera(self):
        ...
        time_reshape_1dim_to_2dim = time.time()
    ...
    def take_picture(self, filename = None):
        ...
        if self.__my_filename_current_image:
            if not filename:
                time_convert_rgb565_to_bgr888 = time.time()
                self.__rgb565_to_bgr888()
                time_convert_rgb565_to_bgr888 = time.time()-time_convert_rgb565_to_bgr888

                # removing the last 4 character of my_filename_current_image
                # because we add the counter in picture name
                counter = '0'# '{:04d}'.format(self.counter_img)
                time_to_write_image = time.time()

                self.__save_bmp_image(
                    self.__my_filename_current_image[:-10] + counter + '.bmp')

                time_to_write_image = time.time()-time_to_write_image
                ...
                with open('results_120_wifi.json', "r") as file:
                    ...
                    data['time_to_write_image'] += [time_to_write_image]
                    data['time_convert_rgb565_to_bgr888'] += [time_convert_rgb565_to_bgr888]
