...
class PiPuckEpuck(Epuck):
    ...
    def go_on(self, clock_speed = None):
        ...
        super_go_on = time.time()
        super().go_on()
        
        super_go_on = (time.time()-super_go_on)
        ...
        update_sensor_data = time.time() - update_sensor_data
        ...
        retrieve_checksum_from_robot = time.time()
        for i in range(I2C_SENSORS_PACKET_SIZE - 1):
            checksum ^= self.sensors_data[i]
        
        retrieve_checksum_from_robot = time.time()-retrieve_checksum_from_robot

        #Should pass this if condition in each loop-
        if  checksum == self.sensors_data[I2C_SENSORS_PACKET_SIZE - 1]:
            
            with open('results_go_on_pipuck_stress.json', "r") as file:
                data = json.load(file)

            data['create_checksum'] += [create_checksum]
            data['update_sensor_data'] += [update_sensor_data]
            data['retrieve_checksum_from_robot'] += [retrieve_checksum_from_robot]

            
            with open('results_go_on_pipuck_stress.json', "w") as file:    
                json.dump(data,file)
    ...
    def get_camera_read(self):
        ...
        with open('pipuck_go_on_benchmark.csv', "rw") as file:
            data = json.load(file)
            data['take_image']+=[diff_time]
            json.dump(data, file)
