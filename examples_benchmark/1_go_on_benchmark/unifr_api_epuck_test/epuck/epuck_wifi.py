...
class WifiEpuck(Epuck):
    ...
    def __receive_from_robot(self):
        ...
        time_to_check_header = time.time()
        header = self.__receive_part_from_robot(self.HEADER_PACKET_SIZE)
        time_to_check_header = time.time()- time_to_check_header

        # camera information
        time_to_receive_image = 0
        time_to_receive_sensor = 0
        if header == bytearray([1]):
            time_to_receive_image = time.time()
            self.__rgb565 = self.__receive_part_from_robot(self.IMAGE_PACKET_SIZE)
            time_to_receive_image = time.time()-time_to_receive_image
            self.__camera_updated = True

        # sensors information
        elif header == bytearray([2]):
            time_to_receive_sensor = time.time()
            self.sensors = self.__receive_part_from_robot(self.SENSORS_PACKET_SIZE)
            time_to_receive_sensor = time.time() - time_to_receive_sensor
        # no information
        else:
            return False


        with open('results_go_on_wifi_with_image.json', "r") as file:
            data = json.load(file)

            if time_to_receive_image != 0:
                data['time_to_receive_image'] += [time_to_receive_image]
            if time_to_receive_sensor != 0:
                data['time_to_receive_sensor'] += [time_to_receive_sensor]
            if time_to_check_header != 0:
                data['time_to_check_header'] += [time_to_check_header]
            
        with open('results_go_on_wifi_with_image.json', "w") as file:    
            json.dump(data,file)
    ...
    def go_on(self):
        ...
        * Sends and receives commands between computer and robot.
        ...
        time_to_send_data = time.time()
        self.__send_to_robot()
        time_to_send_data = time.time()-time_to_send_data
        ...
        with open('results_go_on_wifi_with_image.json', "r") as file:
            ...
            data['time_to_send_data'] += [time_to_send_data]
