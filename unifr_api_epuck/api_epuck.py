...
def __get_robot_webot():
    ...
    try: 
        ...
        print('\033[91m'+'You did not enter an IP address, Please launch the script from Webots.'+'\033[0m')
...
class Epuck:
    ...
    def get_magnetometer(self):
        """
        Raw axes values expressed in float, range is +-4912.0 uT (magnetic flux density expressed in micro Tesla)        
        :returns: values in micro Tesla
        :rtype: float
        """
        pass
    ...
    def play_mario(self):
        """
        .. warning:: On Webots, it is just a playback music, it does not affect the simulation.
        ...
        if self.get_id() == host_creator:
            ...
            while is_online != 0:
                ...
                if time_fail < time.time():
                    print('No communication for '+ self.get_id() + '. Not connected to host manager.')
                    if not host_creator:
                        print('Please create a GUI host by executing on your terminal: `python3 -m unifr_api_epuck`')
        ...
        if self.manager:
            try:
                if self.has_receive_msg():
                    ...
                    self.syncdict.update({self.get_id():current_dict[self.get_id()]})
                    self.lock.release()
                    return recv_mess
...
class __DirectEpuck(Epuck):
    ...
    def __tcp_init(self):
        ...
        print("Try to connect to " + ip_address + ":" + str(TCP_PORT) + " (TCP)")
    ...
    def __receive_part_from_robot(self, msg_len):
        ...
        try:
            ...
            print('\033[91m'+'Lost connection of : '+ str(self.get_ip())+'\033[0m')
        ...
        if led_position in range(LED_COUNT):
            ...
            if led_position % 2 == 0:
                ...
                if red != None and green != None and blue != None:
                    ...
                    between = lambda color_val: 0 <= color_val <= 100
                    in_rgb_range_values = list(map(between, (red,green,blue)))
                    ...
                    if all(in_rgb_range_values):
                        ...
                        for i in range(3):
                            ...
                            if not in_rgb_range_values[i]:
                                print(f'color {color[i]} is not between 0 and 100')
                        
                else:

                    self.command[led_position] = 15 & 0xFF
                    self.command[led_position+1] = 0
                    self.command[led_position+2] = 0
       
        else: 
            print(f'invalid led position: {led_position}. Accepts 0 <= x <= 7. LED stays unchange.')
    ...
    def disable_led(self,led_position):
        if led_position in range(LED_COUNT):
            ...
            print(f'invalid led position: {led_position}. Accepts 0 <= x <= 7. LED stays ON.')
    ...
    def get_prox(self):
        ...
        prox_values[PROX_RIGHT_FRONT] = struct.unpack("H", struct.pack("<BB", sensor[37], sensor[38]))[0]
        prox_values[PROX_RIGHT_FRONT_DIAG] = struct.unpack("H", struct.pack("<BB", sensor[39], sensor[40]))[0]
        prox_values[PROX_RIGHT_SIDE] = struct.unpack("H", struct.pack("<BB", sensor[41], sensor[42]))[0]
        prox_values[PROX_RIGHT_BACK] = struct.unpack("H", struct.pack("<BB", sensor[43], sensor[44]))[0]

        prox_values[PROX_LEFT_BACK] = struct.unpack("H", struct.pack("<BB", sensor[45], sensor[46]))[0]
        prox_values[PROX_LEFT_SIDE] = struct.unpack("H", struct.pack("<BB", sensor[47], sensor[48]))[0]
        prox_values[PROX_LEFT_FRONT_DIAG] = struct.unpack("H", struct.pack("<BB", sensor[49], sensor[50]))[0]
        prox_values[PROX_LEFT_FRONT] = struct.unpack("H", struct.pack("<BB", sensor[51], sensor[52]))[0]
    ...
    def get_lights(self):
        ...
        light_values[PROX_RIGHT_FRONT] = struct.unpack("H", struct.pack("<BB", sensor[53], sensor[54]))[0]
        light_values[PROX_RIGHT_FRONT_DIAG] = struct.unpack("H", struct.pack("<BB", sensor[55], sensor[56]))[0]
        light_values[PROX_RIGHT_SIDE] = struct.unpack("H", struct.pack("<BB", sensor[57], sensor[58]))[0]
        light_values[PROX_RIGHT_BACK] = struct.unpack("H", struct.pack("<BB", sensor[59], sensor[60]))[0]

        light_values[PROX_LEFT_BACK] = struct.unpack("H", struct.pack("<BB", sensor[61], sensor[62]))[0]
        light_values[PROX_LEFT_SIDE] = struct.unpack("H", struct.pack("<BB", sensor[63], sensor[64]))[0]
        light_values[PROX_LEFT_FRONT_DIAG] = struct.unpack("H", struct.pack("<BB", sensor[65], sensor[66]))[0]
        light_values[PROX_LEFT_FRONT] = struct.unpack("H", struct.pack("<BB", sensor[67], sensor[68]))[0]
    ...
    def get_ground(self):
        ...
        ground_values[GS_LEFT] = struct.unpack("H", struct.pack("<BB", sensor[90], sensor[91]))[0]
        ground_values[GS_CENTER] = struct.unpack("H", struct.pack("<BB", sensor[92], sensor[93]))[0]
        ground_values[GS_RIGHT] = struct.unpack("H", struct.pack("<BB", sensor[94], sensor[95]))[0]
    ...
    def get_gyro(self):
        x,y,z = self.get_gyro_axes()
        return [x, y, z]

    def get_gyro_axes(self):
        sensor = self.sensor
        gyro_x = struct.unpack("<h", struct.pack("<BB", sensor[18], sensor[19]))[0]
        gyro_y = struct.unpack("<h", struct.pack("<BB", sensor[20], sensor[21]))[0]
        gyro_z = struct.unpack("<h", struct.pack("<BB", sensor[22], sensor[23]))[0]
    ...
    def get_accelerometer_axes(self):
        sensor = self.sensor        
        axe_x = struct.unpack("<h", struct.pack("<BB", sensor[0], sensor[1]))[0]
        axe_y = struct.unpack("<h", struct.pack("<BB", sensor[2], sensor[3]))[0]
        axe_z = struct.unpack("<h", struct.pack("<BB", sensor[4], sensor[5]))[0]
        return axe_x, axe_y, axe_z

    # return front, right, back. left microphones
    def get_microphones(self):
        sensor = self.sensor
        front = struct.unpack("<h", struct.pack("<BB", sensor[77], sensor[78]))[0]
        right = struct.unpack("<h", struct.pack("<BB", sensor[71], sensor[72]))[0]
        back = struct.unpack("<h", struct.pack("<BB", sensor[75], sensor[76]))[0]
        left = struct.unpack("<h", struct.pack("<BB", sensor[73], sensor[74]))[0]
    ...
    def get_roll(self):
        sensor = self.sensor
        return struct.unpack("f", struct.pack("<BBBB", sensor[10], sensor[11], sensor[12], sensor[13]))[0]


    def get_pitch(self):
        sensor = self.sensor
        return struct.unpack("f", struct.pack("<BBBB", sensor[14], sensor[15], sensor[16], sensor[17]))[0]
    ...
    def live_camera(self,live_time=None):
        ...
        if live_time is None or (self.current_time - self.start_time) < live_time:
            ...
            print(self.get_id() + ' disabling camera.')
            self.disable_camera()
...
class __WebotsEpuck(Epuck):
    ...
    def set_speed(self, speed_left, speed_right=None):
        ...
        self.leftMotor.setVelocity(super().bounded_speed(speed_left))
        self.rightMotor.setVelocity(super().bounded_speed(speed_right))

    def get_motors_steps(self):
        print('Not available on Webots')
        return 
    ...
    def toggle_led(self, led_position, red=None, green=None, blue=None):
        ...
        if led_position in range(LED_COUNT):
            ...
            print(f'invalid led position: {led_position}. Accepts 0 <= x <= 7. LEDs unchanged.')
    ...
    def get_prox(self):
        ...
        try:
            prox_values = [self.ps[i].getValue() for i in range(PROX_SENSORS_COUNT)]
        except:
    ...
    def get_lights(self):
        try:
            ls_values = [self.ls[i].getValue() for i in range(PROX_SENSORS_COUNT)]
            return ls_values
        except:
    ...
    def get_ground(self):
        try:
            ground_values = [self.gs[i].getValue() for i in range(GROUND_SENSORS_COUNT)]
    ...
    def get_gyro_axes(self):
        gyro_axes = self.get_gyro()
        return gyro_axes[0], gyro_axes[1], gyro_axes[2]
    ...
    def take_picture(self):
        try:
            save_as = self.dir +'/image'+str(self.counter_img)+'.jpg'
            print(save_as)
    ...
    def get_camera(self):
        ...
        for n in range(CAMERA_WIDTH):
            for m in range(CAMERA_HEIGHT):
                ...
                self.red += [self.camera.imageGetRed(cameraData, CAMERA_WIDTH, n, m)]
    ...
    def play_mario(self):
        pass
        #Process(target=playsound, args=('./music/play_mario.wav',)).start()     

    def play_Underworld(self):
        pass
        #Process(target=playsound, args=('./music/play_underworld.wav',)).start()

    def play_star_wars(self):
        pass
        #Process(target=playsound, args=('./music/play_star_wars.wav',)).start()
